...
def norm(observed,
    ...
    norm = np.nansum([ (1+o)/e for b,o,e in zip(bins,observed,errors)]) -7000
...
def norm(observed,
    ...
    if verbose:
        print("Effective number of bins: ", len(observed))
...
def two_point(data, bins, method='standard',
    ...
    KDT_R = BallTree(data_R, metric = "euclidean")
...
def correlate_and_plot(data = list,
    ...
    bins = np.logspace(np.log10(max_angle/bin_number)/np.log10(base),
                       np.log10(max_angle),
                       bin_number,
                       base = base)
    ...
    corr, corr_error =  two_point(data,
            data_R = background,
            bins = bins, 
            method=method,
            random_state=42)
...
def _single_bootstrap(i, representations, sub_sample, epoch, background_factor, verbose):
    ...
    tpcf_score = correlate_and_plot(val_flat_sample,
                                     ...
                                     fig_name="plots/2PCR@Epoch: " + str(epoch) + "_boot" + str(i),
                                     precomputed_RR=None,
                                     bin_number=15,
                                     method="standard",
                                     bootstrap=False,
...
def TPCF_score(representations,
    ...
    #The input is hi-dimesional representations, the 2PCF score is computed on the first 2 PCA components
    ...
    norm_score = Parallel(n_jobs=n_jobs)(
        delayed(_single_bootstrap)(i, representations, sub_sample, epoch, background_factor, verbose)
        for i in range(Nbootstrap)
    )

    return (round(np.ma.masked_invalid(norm_score).mean(0), 2),
            round(np.ma.masked_invalid(norm_score).std(0, ddof=1), 2))
"""
def TPCF_score(representations,
               epoch = 0,
               background_factor =5,
    ...
    for i in range(Nbootstrap):
        ...
        tpcf_score = _single_bootstrap(i, 
                                       representations,
                                       sub_sample, 
                                       epoch, 
                                       background_factor, 
