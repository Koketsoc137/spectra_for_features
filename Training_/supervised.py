...
def galaxyzoo10(config):
    ...
    if data_config["perturb_labels"]:
        labels = perturb_list_by_swapping(labels, percentage=data_config["perturb_percentage"])
    ...
    dataset_split = cust.train_val_dataset(transformed_dataset, train_size = data_config["train_split"],val_split=data_config["val_split"])
...
def train_resnet(config):
    ...
    representations_dir = Path(__file__).resolve().parents[1] / path_config["representations_dir"]
    ...
    train_loader, test_loader = galaxyzoo10(config)
    
    model = models.efficientnet_b0(weights = model_config["weights"])
    ...
    pkl_filename = representations_dir / f"{artifact_prefix}_test_representations_labels{epoch}.csv"
    with open(pkl_filename, 'wb') as file:
        pickle.dump((test_representations,test_labels),file)
            
    pkl_filename = representations_dir / f"{artifact_prefix}_train_representations_labels{epoch}.csv"
    with open(pkl_filename, 'wb') as file:
    ...
    for epoch in range(1,num_epochs):
        ...
        if epoch%training_config["checkpoint_interval"] ==0:
            pkl_filename = output_dir / f"{artifact_prefix}_train_test_TPCF_score.csv"
            with open(pkl_filename, 'wb') as file:
                pickle.dump(TPCF_scores,file)
                
            pkl_filename = output_dir / f"{artifact_prefix}_train_val_accuracy_loss.csv"
            with open(pkl_filename, 'wb') as file:
                pickle.dump(train_val_accuracy_loss,file)
                
            pkl_filename = output_dir / f"{artifact_prefix}_train_val_id_score.csv"
            with open(pkl_filename, 'wb') as file:
