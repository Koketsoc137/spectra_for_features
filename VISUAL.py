...
# Got this directly from latex using \printinunitsof{in}\prntlen{\columnwidth}\\
# and import the layouts package
textheight = 9.0971
textwidth = 7.1014 
columnwidth = 3.35289 
...
if plot_type == 'pdf':
    ...
    figsize_default = figsize_fullwidth
    
elif plot_type == 'paper':
    params['font.family'] = 'Times New Roman'
    params['savefig.format'] = 'pdf'
    params['text.usetex'] = True
    figure_suffix = '.pdf'
    fig_dir = '../MGCLS_anomalies_paper/figures/'
    figsize_default = figsize_square
...
def  umap(dataset,scatter = True,name = "UMAP Distribution", dim = 2, min_dist = 0.1, n_neighbors = 20,alpha = 0.2, low_memory = False):
    start = time.time()
    reducer = UMAP.UMAP(a=None, angular_rp_forest=False, b=None,
     force_approximation_algorithm=False, init='spectral', learning_rate=1.0,
     local_connectivity=1.0, low_memory=low_memory, metric='euclidean',
...
def shade(embedded_dataset,
    ...
    for i in range(numof_class):
        try:
            ...
            plt.scatter(classes[i][:,0],classes[i][:,1],s = size,c = col,alpha = alpha,marker =marker)
...
def show_images(image_inputs = [],indices = [],n_by_n = True,sqrt = 8,label_1 = None,label_2 = None):
    ...
    grid = ImageGrid(fig, 111,  # similar to subplot(111)
                     ...
                     axes_pad=.01,  # pad between axes in inch.
                     share_all = True)
    ...
    for ax, im in zip(grid, images):
        ...
        ax.set_ylabel(label_1[a],fontsize = 22)
        ax.set_xlabel(label_2[b], fontsize = 22)
        b+=1
        if b >2:
            a+=1
            b=0
